...
{ 
    ...
    switch (landingOnVehicle_state)
    {
        case FOLLOWING:
            follow_target_3D();
            if (target_was_acquired == true && target_over_vehicle_has_been_reached() ) { landingOnVehicle_state = READY_FOR_DESCENT; }
            break;
...
void ModeThrow::follow_target_3D()
{
    ...
    if (g2.follow.get_target_dist_and_vel_ned(dist_vec, dist_vec_offs, vel_of_target) && allow_following)
    {
        ...
        else { target_speed_bearing = 0; } // Speed not high enough to determine a proper target speed bearing
    }
